...
In healthy adults, REM sleep typically constitutes approximately 20--25\% of total sleep duration \cite{carskadon2017sleep}. As the data shows a mean sleep duration of 6.89 hours, this proportion corresponds to an expected REM sleep duration of approximately
\[
...
The dataset consists of 247 sleep observations collected between November 2024 and December 2025 using a Samsung Galaxy Watch.
...
Month-Year & N & Mean & Std & N & Mean & Std \\ \midrule
...
12-2025 & 10 & 1.86 & 0.38 & 13 & 10,716 & 4,746 \\ \bottomrule
\end{tabular}%
}
\end{table}
...
Table \ref{tab:stats} summarizes the descriptive statistics of the key variables.
...
\begin{tabular}{cccc}
\toprule
\textbf{Statistic} & \textbf{Total Sleep Hours} & \textbf{REM Hours ($Y$)} & \textbf{Step Count ($X$)} \\
\midrule
N         & 247    & 247    & 247    \\
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.7\linewidth]{dist_daily_rem_hours_MLE.png}
...
\begin{figure}[H]
    \centering
    % Updated filename
    \includegraphics[width=0.4\linewidth]{boxplot_daily_rem_hours.png}
    \caption{Boxplot of Daily REM Sleep Hours.}
    \label{fig:box}
\end{figure}

\subsection{Correlation Analysis}
The Pearson correlation coefficient between daily step count and REM sleep is $r = 0.237$, indicating a weak positive linear relationship. Fig. \ref{fig:scatter} illustrates this trend with the regression line overlaid.
...
We adopted a Bayesian framework with a conjugate Normal prior for the mean REM sleep duration. 
The prior mean was set to $\mu_0 = 1.5$ hours with prior standard deviation $\tau = 0.5$, representing a weakly informative prior centered at the benchmark value introduced earlier.
...
\bibitem{carskadon2017sleep}
M.~A. Carskadon and W.~C. Dement,
``Normal human sleep: An overview,''
in \emph{Principles and Practice of Sleep Medicine},
6th~ed., M.~H. Kryger, T.~Roth, and W.~C. Dement, Eds.
Philadelphia, PA, USA: Elsevier, 2017, pp.~15--24.
